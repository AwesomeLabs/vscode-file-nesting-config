...
const tsconfig = ["tsconfig.*", "tsdoc.*", "jsconfig.*", "api-extractor.json"];
...
const env = ["*.env", ".env.*", "env.d.ts", ".envrc"];
...
const docker = ["dockerfile*", ".dockerignore", "docker-compose.*"];
...
const gofile = ["go.sum", ".air*"];

const gemfile = ["gemfile.lock", ".ruby-version"];

const composer = ["composer.lock", "phpunit.xml*", "psalm*.xml", ".php*.cache"];
...
const pdm = ["pyproject.toml", "pdm.lock", ".pdm.toml"];
...
const denoRuntime = ["import_map.json", "import-map.json", ...tsconfig, ...env];
...
const svelteKitRouting = {
  ...
  "+layout.svelte":
    "+layout.ts,+layout.ts,+layout.js,+layout.server.ts,+layout.server.js",
};
...
const full = sortObject({
  ...
  ...Object.fromEntries(
    Object.entries(frameworks).map(([n, i]) => [
      n,
      stringify([...i, ...libraries]),
    ])
  ),
  ...svelteKitRouting,
});
...
fs.writeFileSync(
  "README.md",
  fs.readFileSync("README.md", "utf-8").replace(/```json([\s\S]*?)```/m, () => {
